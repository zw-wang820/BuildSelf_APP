...
/// 生活统计页 — 按时间窗口统计生活记录，按分类聚合
class LifeStatsScreen extends StatefulWidget {
  const LifeStatsScreen({Key? key}) : super(key: key);

  @override
  State<LifeStatsScreen> createState() => _LifeStatsScreenState();
}

class _LifeStatsScreenState extends State<LifeStatsScreen> {
  final _repo = LifeRepository();
  ...
  Future<void> _loadData() async {
    try {
      final records = await _repo.getAll(_userId);
      if (mounted) {
        setState(() {
          _records = records;
          _loading = false;
  ...
  /// 窗口内创建的生活记录
  List<LifeRecord> get _windowRecords {
    final range = _range;
    final start = range[0];
    final end = range[1];
    return _records
  ...
  int get _total => _windowRecords.length;

  /// 窗口内涉及的分类数
  int get _categoryCount =>
      _windowRecords.map((r) => r.recordType).toSet().length;
  ...
  List<_CategoryStat> get _categoryStats {
    ...
    for (final r in _windowRecords) {
      counts[r.recordType] = (counts[r.recordType] ?? 0) + 1;
    }
    final stats = counts.entries
        .map((e) => _CategoryStat(type: e.key, count: e.value))
        .toList();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('生活统计')),
  ...
  Widget _buildSummary(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: AppColors.life.withValues(alpha: 0.07),
        borderRadius: BorderRadius.circular(16),
      ),
      child: Row(
        children: [
          _summaryItem(context, '$_total', '记录总数', AppColors.life),
          _summaryItem(
              context, _dailyAvg.toStringAsFixed(1), '日均记录', AppColors.info),
          _summaryItem(context, '$_categoryCount', '涉及分类', AppColors.warning),
        ],
      ),
    );
  }
  ...
  Widget _buildCategoryBar(BuildContext context, _CategoryStat stat) {
    final color = lifeTypeColor(stat.type);
      ...
      children: [
        Row(
          children: [
            TextButton.icon(
              ...
              style: TextButton.styleFrom(foregroundColor: AppColors.life),
            ),
            const Spacer(),
            Text(
              '${lifeTypeEmoji(type)} $type · ${list.length} 条',
              style: TextStyle(
                fontSize: 13,
                fontWeight: FontWeight.w600,
                color: AppColors.textPrimary(context),
        ...
        if (list.isEmpty)
          ...
          for (final record in list) ...[
            _buildRecordRow(context, record),
            const SizedBox(height: 10),
          ],
      ],
    );
  ...
  Widget _buildRecordRow(BuildContext context, LifeRecord record) {
    final color = lifeTypeColor(record.recordType);
    return AppCard(
      accent: color,
      ...
      child: Row(
        children: [
          Text(lifeTypeEmoji(record.recordType),
  ...
  /// 轻量详情弹层（与生活列表点卡片一致）
  void _showDetailSheet(BuildContext context, LifeRecord record) {
            ...
            children: [
              Row(
                children: [
                  ...
                  const SizedBox(width: 8),
                  Text(
                    '${lifeTypeEmoji(record.recordType)} ${record.recordType}',
...
class _CategoryStat {
  ...
  _CategoryStat({required this.type, required this.count});
}
